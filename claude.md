...
- **`credentials: 'include'`** solo es necesario en las llamadas a `/api/auth/refresh` y `/api/auth/logout` (para que viaje la cookie httpOnly cross-domain). El resto de llamadas autenticadas usan el header `Authorization: Bearer <token>` normal, sin necesidad de `credentials`.
